...
export default class Gaming extends Component {
    ...
    ConnectWebSocket() {
        ...
        this.ws.onmessage = (e) => {
            ...
            switch (res.code) {
                ...
                case TAKE_PHOTO_AIM: {
                    alert("请在光线明亮的地方拍摄"+res.playername+"全身像，以录入信息。");
                    this.setState({
                        take_photo_aim : res.playername
                    })
                    break;
                }
                case POSITION: {
                    ...
                    if (person.confidence > 85) {
                        if (this.state.stage == 0 || this.state.stage == 1 && this.state.take_photo_aim) {
                            ...
                            let data = {
                                ...
                                playername: this.state.take_photo_aim,
    ...
    render() {
        ...
        if(this.state.Using_map) return(<BaiduMap teammates_position={this.state.teammates_position} UnUse_Map={this.UnUse_Map} center={this.state.center}  />);
